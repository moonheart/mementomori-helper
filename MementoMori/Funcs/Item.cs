...
public partial class MementoMoriFuncs
{
    public async Task AutoUseItems()
    {
        await ExecuteQuickAction(async (log, token) =>
        {
            ...
            log(Masters.TextResourceTable.Get("[CharacterResetErrorMessageNotEnoughDiamond]"));
        });
        ...
        async Task OpenTreasure(long openCount, TreasureChestMB treasureChestMb, Action<string> log)
        {
            ...
            log($"{Masters.TextResourceTable.Get("[CommonOpenLabel]")} {Masters.TextResourceTable.Get(treasureChestMb.NameKey)} x {openCount}");
