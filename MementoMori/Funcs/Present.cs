        ...
        await ExecuteQuickAction(async (log, token) =>
        {
            do
            {
                ...
                if (getListResponse.userPresentDtoInfos.Any(d => !d.IsReceived))
                {
                    ...
                    {
                        log(e.Message);
                        // if there are no QuestQuickTicket or Equipment, break the loop
                        var grp = getListResponse.userPresentDtoInfos.SelectMany(d => d.ItemList).GroupBy(d => new {d.Item.ItemType, d.Item.ItemId});
                        if (grp.Count(d => d.Key.ItemType == ItemType.QuestQuickTicket || d.Key.ItemType == ItemType.Equipment) == 0) break;
                    }
                }
                else
                {
