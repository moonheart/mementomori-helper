...
public partial class MementoMoriFuncs
{
    public async Task GetAutoBattleReward()
    {
        await ExecuteQuickAction(async (log, token) =>
        {
            ...
            log(Masters.TextResourceTable.Get("[AutoBattleRewardInfoTitle]"));
            log($"{Masters.TextResourceTable.Get("[CommonWinLabel]")} {bonus.AutoBattleRewardResult.BattleCountWin}");
            log($"{Masters.TextResourceTable.Get("[CommonLoseLabel]")} {bonus.AutoBattleRewardResult.BattleCountAll - bonus.AutoBattleRewardResult.BattleCountWin}");
            log($"{Masters.TextResourceTable.Get("[AutoBattleRewardInfoBattleTotalTimeLabel]")} {TimeSpan.FromMilliseconds(bonus.AutoBattleRewardResult.BattleTotalTime)}");
            log(Masters.TextResourceTable.Get("[AutoBattleRewardInfoPopulationLabel]"));
            log($"{Masters.TextResourceTable.Get("[ItemName5]")} {bonus.AutoBattleRewardResult.GoldByPopulation}");
            log($"{Masters.TextResourceTable.Get("[ItemName11]")} {bonus.AutoBattleRewardResult.PotentialJewelByPopulation}");
            log(Masters.TextResourceTable.Get("[AutoBattleRewardInfoRewardLabel]"));
        ...
        await ExecuteQuickAction(async (log, token) =>
        {
            try
            {
                if (IsBossBattleQuickAvailable)
                {
                    ...
                    log($"{Masters.TextResourceTable.Get("[AutoBattleButtonQuickForward]")}:\n");
    ...
    private bool IsBossBattleQuickAvailable
    {
        get
        {
            ...
            if (!isQuickAvailable)
            {
                isQuickAvailable = UserSyncData.UserBattleBossDtoInfo.BossClearMaxQuestId >= Masters.OpenContentTable.GetByOpenCommandType(OpenCommandType.BossBattleQuick).OpenContentValue;
            }
    ...
    public async Task BossHishSpeedBattle()
    {
        await ExecuteQuickAction(async (log, token) =>
        {
            ...
            if (await IsValidMonthlyBoost())
            {
                ...
                if (availableCount > 0)
                {
                    log(Masters.TextResourceTable.Get("[MonthlyBoostPrivilegeDescription3]"));
            ...
            {
                log(Masters.TextResourceTable.GetErrorCodeMessage(ErrorCode.BattleAutoNotEnoughPrivilegeCount));
            }
            else
            {
                log($"{Masters.TextResourceTable.Get("[AutoBattleButtonQuickForward]")}{Masters.TextResourceTable.Get("[CommonRewardLabel]")}:\n");
        ...
        async Task BattleQuick(Action<string> log, QuestQuickExecuteType type, int count)
        {
            ...
            log($"{Masters.TextResourceTable.Get("[AutoBattleRewardInfoPopulationLabel]")} {Masters.TextResourceTable.Get("[ItemName5]")} {quickResponse.AutoBattleRewardResult.GoldByPopulation}");
            log($"{Masters.TextResourceTable.Get("[AutoBattleRewardInfoPopulationLabel]")} {Masters.TextResourceTable.Get("[ItemName11]")} {quickResponse.AutoBattleRewardResult.PotentialJewelByPopulation}");
            log($"{Masters.TextResourceTable.Get("[ItemName6]")} {quickResponse.AutoBattleRewardResult.BattleRewardResult.CharacterExp}");
            log($"{Masters.TextResourceTable.Get("[ItemName5]")} {quickResponse.AutoBattleRewardResult.BattleRewardResult.ExtraGold}");
            log($"{Masters.TextResourceTable.Get("[ItemName11]")} {quickResponse.AutoBattleRewardResult.BattleRewardResult.PlayerExp}");
            log($"{Masters.TextResourceTable.Get("[CharacterLevelUpLabel]")} {quickResponse.AutoBattleRewardResult.BattleRewardResult.RankUp}");
